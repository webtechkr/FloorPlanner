(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "../static/js/go", "./GoCloudStorageManager", "./GoDropBox", "./GoGoogleDrive", "./GoLocalStorage", "./GoOneDrive"], factory);
    }
    ...
    function init() {
        ...
        window.handlePromise = function (action) {
            ...
            switch (action) {
                ...
                case 'Delete':
                    ...
                    break;
                case 'New':
                    storageManager.create(myDiagram.isModified).then(function (fileData) {
                        handleFileData(action, fileData);
                    });
                    break;
                case 'SaveAs':
        ...
        // gls.loadWithUI();
        // by default, go ahead and load "Default" via GoLocalStorage, if it exists
        var defaultName = "Default.diagram";
        if (storageManager.currentStorage instanceof GoLocalStorage_1.GoLocalStorage) {
            if (window.localStorage["Default.diagram"]) {
                gls.load(defaultName).then(function () {
                    updateTitle();
                });
            }
            else {
                // if not yet saved to localStorage, save the default model as Default.diagram, then load it
                gls.save(defaultName).then(function () {
                    gls.load(defaultName).then(function () {
                        updateTitle();
                    });
                });
